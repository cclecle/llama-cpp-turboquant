...
public:
    ...
    // host bank when the cache is positionally split (-nckvc)
    ggml_tensor * get_k_idxs_host() const { return self_k_idxs_host; }
    ggml_tensor * get_kq_mask_host() const { return self_kq_mask_host_cnv; }
...
public:
    ...
    // host bank of the base cache when it is positionally split (-nckvc). the SWA cache is
    // never split, so it has no host half
    ggml_tensor * get_k_idxs_host() const { return self_k_idxs_host; }
    ggml_tensor * get_v_idxs_host() const { return self_v_idxs_host; }
    ggml_tensor * get_kq_mask_host() const { return self_kq_mask_host_cnv; }
