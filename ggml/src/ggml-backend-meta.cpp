...
static struct ggml_backend_meta_split_state ggml_backend_meta_get_split_state(
    ...
    auto handle_flash_attn_ext = [&](const std::vector<ggml_backend_meta_split_state> & src_ss) -> ggml_backend_meta_split_state {
        ...
        // Head-split Q against a MIRRORED multi-head KV is NOT correct and must never silently
        // proceed: FlashAttention derives the KV head from the Q head index *local to the tensor it
        ...
        if (q_head_split && kv_mirrored) {
            GGML_LOG_ERROR("%s: KV is MIRRORED with %lld heads while Q is head-split. This is not "
                           "supported and would produce incorrect results. Use a matched K/V cache "
                           "quant type (so a fused FA kernel exists, e.g. -ctk q4_0 -ctv q4_0), build "
                           "with GGML_CUDA_FA_ALL_QUANTS=ON, and do not use --no-kv-offload with "
                           "-sm tensor.\n",
                           __func__, (long long) tensor->src[1]->ne[2]);
            GGML_ABORT("handle_flash_attn_ext: head-split Q with MIRRORED multi-head KV is unsupported");
        }
