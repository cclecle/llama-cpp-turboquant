...
enum common_speculative_type {
    ...
    COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE,   // Eagle-1/2 speculative decoding (e.g. Mistral EAGLE)
    ...
    uint32_t need_n_rs_seq() const {
        bool needs_rs_seq = std::any_of(types.begin(), types.end(), [&](auto t) {
            return t == COMMON_SPECULATIVE_TYPE_DRAFT_MTP || t == COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE3 ||
                   t == COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE || t == COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH ||
...
struct common_params {
    ...
    int32_t n_cpu_kv_layers = 0;   // keep the KV of this many layers in host RAM (0 = all in VRAM)
    int32_t n_cpu_kv_cells  = 0;   // keep the last N cells of every slot's KV in host RAM (0 = all in VRAM)
    ...
    bool    mtmd_checkpoints    = false; // allow context checkpoints on batches that processed media
